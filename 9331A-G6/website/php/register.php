...
if(isset($_POST['submit'])){
    ...
    if(empty($firstName) || empty($lastName) || empty($email)  || empty($userName) || empty($pwd) || empty($repwd)){
        ...
        if(!preg_match("/^[a-zA-z]*$/",$firstName)|| !preg_match("/^[a-zA-z]*$/",$lastName)){
            ...
            if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
                ...
                if($resultCheck > 0){
                    ...
                    $sql ="INSERT INTO users (fname, lname, uname, emailadd, birthday, pass, repass) 
                        VALUES ('$firstName', '$lastName','$userName','$email','$bday','$hashedPwd','$hashedRepwd');";
                    $result = mysqli_query($conn, $sql);
